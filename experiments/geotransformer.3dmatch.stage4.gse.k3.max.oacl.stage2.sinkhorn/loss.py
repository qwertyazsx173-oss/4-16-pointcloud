...
class ReliabilityConsistencyLoss(nn.Module):
    def __init__(self, cfg):
        super(ReliabilityConsistencyLoss, self).__init__()
        self.positive_radius = cfg.fine_loss.positive_radius

    def forward(self, output_dict, data_dict):
        ref_node_corr_knn_points = output_dict['ref_node_corr_knn_points']      # (P, K, 3)
        src_node_corr_knn_points = output_dict['src_node_corr_knn_points']      # (P, K, 3)
        ref_node_corr_knn_masks = output_dict['ref_node_corr_knn_masks']        # (P, K)
        src_node_corr_knn_masks = output_dict['src_node_corr_knn_masks']        # (P, K)
        node_corr_reliability = output_dict['node_corr_reliability']            # (P,)
        transform = data_dict['transform']

        src_node_corr_knn_points = apply_transform(src_node_corr_knn_points, transform)
        dists = pairwise_distance(ref_node_corr_knn_points, src_node_corr_knn_points)  # (P, K, K)

        gt_masks = torch.logical_and(
            ref_node_corr_knn_masks.unsqueeze(2),
        ...
        valid_pair_count = gt_masks.float().sum(dim=(1, 2)).clamp(min=1.0)      # (P,)
        positive_pair_count = gt_corr_map.float().sum(dim=(1, 2))                # (P,)
        target_reliability = positive_pair_count / valid_pair_count              # (P,)

        loss = F.binary_cross_entropy(
            node_corr_reliability.clamp(min=1e-6, max=1.0 - 1e-6),
            target_reliability.detach(),
        )
        return loss
